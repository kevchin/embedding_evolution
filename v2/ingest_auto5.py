...
CAPTION_BATCH_SIZE = 50     # safe at 512×512 on 16 GB; lower to 1-2 if OOM
...
def find_input_images(directory: str) -> list[str]:
    ...
    Renames any uppercase extensions (.JPG → .jpg) in place before returning."""
    paths = sorted(
        p
    ...
    for p in paths:
        if p.suffix != p.suffix.lower():
            normalized = p.with_suffix(p.suffix.lower())
            if not normalized.exists():
                p.rename(normalized)
                p = normalized
            else:
                log.warning("Cannot normalize extension (target exists): %s", p)
        result.append(str(p))
    return result
